...
- **Material pipeline**:
  ...
  - Transparency (cutout-precedence as of 2026-05-28): **Cutout wins over Blended.** `_AlphaClip==1` or `_Mode==1` → `opacity.mode="Cutout"`; else `_Surface==1` or `_Mode>=2` → `"Blended"`. Materials in the wild set BOTH (Office `MetalShelf`: `_AlphaClip=1`+`_Mode=1`+`_Surface=1`); reading `_Surface` first made them Blended → fully invisible. Cutout fails safe. Logic in `platforms/unity/asset_database.py::_extract_material_data`; tests `tests/unit/test_material_opacity_mode.py`
...
- **PhysX mesh collider** (2026-05-28): MeshCollider entities cook a real `.pxmesh` via a **per-sub-mesh** model. A collider's geometry is `m_Mesh = {guid, fileID}` → one FBX node → one PhysX MeshGroup → one pxmesh; many groups may share an assetinfo. Two axes handled: ArtFBX↔ColliderFBX (dedicated `_COL` FBXs are scraped + given a physics-only assetinfo) and Mesh→SubMesh (multi-LOD / multi-submesh `_COL`). fileID→node via real render-correlation → single-mesh node[0] → linear `(fileID-4300000)//2` decode. Groups are TriMesh by default / `export method:1` for convex. State index stores `physx_specs` (replaced `collider_entity_node_map`); component hint via `ctx.collider_pxmesh_mapping[(guid,fileID)]`. Proven end-to-end on Alien Forest (T2), NatureManufacture (T3), Office (T4). Engine-source-verified. See `mem:physx_mesh_collider/*` + `mem:asset_packs/import_catalogue`. In-engine cook verification still pending.
