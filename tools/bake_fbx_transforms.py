...
This replicates Unity's FBX import behavior: meshes end up centered on
their object origin so that O3DE entities display them at the correct
position.

STANDALONE TOOL — runs inside Blender's Python (requires `bpy`). NOT
imported by the live converter pipeline. Invoke manually when an FBX
needs its node-space transforms collapsed into vertex data::

    blender --background --python tools/bake_fbx_transforms.py
